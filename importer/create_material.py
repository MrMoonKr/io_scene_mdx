...
def create_material( model: WarCraft3Model, team_color: str ) -> list[Material]:
    ...
    for material in model.materials:
        bpy_images_of_layer: list[Image] = [] # 텍서쳐 레이어 모으기
        for layer in material.layers:
            #bpy_images_of_layer.append( bpy_images[ layer.texture_id ] )
            bpy_images_of_layer.append( bpy_images[ layer.map_ids[0] ] )
...
def create_bpy_material( bpy_images_of_layer: list[Image], material: WarCraft3Material ) -> Material:
    ...
    material_name           = bpy_images_of_layer[0].filepath.split( os.path.sep )[-1].split('.')[0]
...
def get_image( folders: list[str], team_color: str, texture: WarCraft3Texture, texture_exc: str ) -> Image:
    ...
    print( "[io_scene_mdx] image loading : ", file_name, " : ", image_file )
...
def get_folders( alt_folder: str, resource_folder: str, model_file: str ) -> list[str]:
    ...
    if alt_folder == '':
        print( "[io_scene_mdx] No alt resource folder set in addon preferences" )
    ...
    folders1 = [ model_folder,
                 model_folder + textures1, 
                 model_folder + textures2,
                 resource_folder, 
                 alt_folder,
                 resource_folder + textures1, 
                 resource_folder + textures2,
                 alt_folder + textures1,
                 alt_folder + textures2 ]
    folders = []
...
def check_file_path( folder: str, image_file: str ) -> str:
    ...
    try:
        if Path( file_path ).exists():
            print("[io_scene_mdx] got valid : ", file_path )
            return file_path
    except OSError:
        print("[io_scene_mdx] bad path:", file_path )
    return ''
