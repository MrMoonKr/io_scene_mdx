...
def parse_geometry( data: bytes, version: int ) -> WarCraft3Geoset:
    ...
    for chunk in chunks_to_skip:
        chunk_id = br.getid( chunk[0] )
        chunk_data_count = br.getf('<I')[0]

        for _ in range( chunk_data_count ):
            chunk_data = br.getf( chunk[1] )
    ...
    # parse vertex groups
    chunk_id = br.getid( constants.CHUNK_VERTEX_GROUP )
    ...
    # parse matrix Groups
    chunk_id = br.getid( constants.CHUNK_MATRIX_GROUP )
    ...
    chunk_id = br.getid( constants.CHUNK_VERTEX_TEXTURE_POSITION )
